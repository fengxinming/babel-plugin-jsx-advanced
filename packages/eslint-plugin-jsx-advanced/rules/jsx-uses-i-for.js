...
const { OPERATOR_IN, I_FOR } = require('../utils/constants');

module.exports = {
  meta: {
    docs: {
      description: 'a valid expression for i-for',
    ...
    schema: [],
    messages: {
      missingExp: `Missing a expression for i-for.
缺少表达式对于 i-for 指令。
...
e.g.: 
  ...
  i-for={item in items}
`,
      missingVars: `Missing variable declarations for i-for.
缺少变量定义对于 i-for 指令。
...
e.g.: 
  ...
  i-for={item in items}
`,
      missingSpaceBeforeIn: `Missing space before 'in' syntax for i-for.
...
e.g.: 
  ...
  i-for={item in items}
`,
      missingSpaceAfterIn: `Missing space after 'in' syntax for i-for.
